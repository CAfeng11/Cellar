...
## [2.2.3] - 2026-09-20（修订版，build 2）

### 修复

- 识别 Xcode 许可未接受的故障，在主界面集中显示原因、影响、处理步骤及可复制命令。
- 运行时区分未检查、扫描中、成功与失败；失败后的旧快照不再被标记为当前正常状态。
- Homebrew 检查失败优先于空列表和未检查提示；同一次失败只显示一张完整诊断卡。
- 最近任务显示可读结论，原始错误保留为展开详情。

### 发布

- 延续 Universal macOS 未签名、未公证预览版，最低 macOS 14；不自动接受 Xcode 许可或执行管理员命令。
